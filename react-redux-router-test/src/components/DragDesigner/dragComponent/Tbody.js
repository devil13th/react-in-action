...
class Tbody extends CommonComponent{
    constructor(props){
        super(props);
    ...
    static drawComponent = (componentCfg) => {
        console.log(' 开始绘制[tbody] ');
        if(componentCfg.childrens && componentCfg.childrens.length > 0){
            return (
                <TbodyComponent key={componentCfg.id} id={componentCfg.id} data={componentCfg}>
            ...
            return <TbodyComponent key={componentCfg.id} id={componentCfg.id} data={componentCfg}></TbodyComponent>;
        }
    }


    static createInstanceData = (cfg) => {
        console.log(' 创建[tbody]实例数据 ',cfg);
        var id = uuid();
        return {           
            id:id,
            componentId:"Tbody",
...
const TbodyComponent = connect(mapStateToProps,mapDispatchToProps)(Tbody)

export {
    TbodyComponent as Tbody
} ;
