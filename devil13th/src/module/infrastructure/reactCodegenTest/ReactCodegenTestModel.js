...
import {
    fetchQueryReactCodegenTest,
    fetchSaveReactCodegenTest,
    fetchDeleteReactCodegenTest,
    fetchGetReactCodegenTest,
    fetchUpdateReactCodegenTest,
    fetchDeleteReactCodegenTestBatch
} from './ReactCodegenTestService'



export default {

    namespace: 'reactCodegenTestModel',
    ...
    effects: {
         ...
         * @param {*} param1 
         */
        *editReactCodegenTest({ payload }, { call, put, select }) {
            const data = yield call(fetchGetReactCodegenTest, payload);
            if (data.errMessage) {
         ...
         * @param {*} param1 
         */
        *deleteReactCodegenTest({ payload }, { call, put, select }) {
            const data = yield call(fetchDeleteReactCodegenTest, payload);
            if (data.errMessage) {
                message.error("Error : " + data.errMessage);
            } else {
                message.success(" OK !");
         ...
         * @param {*} param1 
         */
        *deleteReactCodegenTestBatch({ payload }, { call, put, select }) {
            const ids = payload.join(",");
            const data = yield call(fetchDeleteReactCodegenTestBatch, ids);
            if (data.errMessage) {
                message.error("Error : " + data.errMessage);
            } else {
                message.success(" OK !");
         ...
         * @param {*} param1 
         */
        *saveReactCodegenTest({ payload }, { call, put, select }) {
            let operateType = yield select((state) => {
                return state.reactCodegenTestModel.operateType
            });

            let operateFn = null;
            if (operateType === CFG.OPERATETYPE.UPDATE) {
                operateFn = fetchUpdateReactCodegenTest;
            } else if (operateType === CFG.OPERATETYPE.SAVE) {
                operateFn = fetchSaveReactCodegenTest;
            } else {
            ...
            if (data.errMessage) {
                ...
                yield put({
                    type: "mergeState",
                    payload: { reactCodegenTestFormVisible: false }
                });
                yield put({ type: "queryList" });
            }
        },
        ...
        *queryList({ payload }, { call, put, select }) {
            ...
            if (payload && payload.queryExtraBean) {//如果payload中有queryExtraBean则设置到state中
                ...
                queryExtraBean = yield select((state) => {
                    return state.reactCodegenTestModel.queryExtraBean;
                });
            ...
            if (payload && payload.queryConditionBean) { //如果payload中有queryConditionBean则设置到state中
                ...
                queryConditionBean = yield select((state) => {
                    return state.reactCodegenTestModel.queryConditionBean;
                });
            ...
            //fetch查询数据
            const data = yield call(fetchQueryReactCodegenTest, queryExtraBean, realConditionBean);
