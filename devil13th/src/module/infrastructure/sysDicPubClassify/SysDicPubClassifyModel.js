...
import {
    fetchQuerySysDicPubClassify,
    fetchSaveSysDicPubClassify,
    fetchDeleteSysDicPubClassify,
    fetchGetSysDicPubClassify,
    fetchUpdateSysDicPubClassify,
    fetchDeleteSysDicPubClassifyBatch
} from './SysDicPubClassifyService'



export default {

    namespace: 'sysDicPubClassifyModel',
    ...
    effects: {
         ...
         * @param {*} param1 
         */
        *editSysDicPubClassify({ payload }, { call, put, select }) {
            const data = yield call(fetchGetSysDicPubClassify, payload);
         ...
         * @param {*} param1 
         */
        *deleteSysDicPubClassify({ payload }, { call, put, select }) {
            const data = yield call(fetchDeleteSysDicPubClassify, payload);
            if (data.errMessage) {
                message.error("Error : " + data.errMessage);
            } else {
                message.success(" OK !");
         ...
         * @param {*} param1 
         */
        *deleteSysDicPubClassifyBatch({ payload }, { call, put, select }) {
            const ids = payload.join(",");
            const data = yield call(fetchDeleteSysDicPubClassifyBatch, ids);
            if (data.errMessage) {
                message.error("Error : " + data.errMessage);
            } else {
                message.success(" OK !");
         ...
         * @param {*} param1 
         */
        *saveSysDicPubClassify({ payload }, { call, put, select }) {
            let operateType = yield select((state) => {
                return state.sysDicPubClassifyModel.operateType
            });

            let operateFn = null;
            if (operateType === CFG.OPERATETYPE.UPDATE) {
                operateFn = fetchUpdateSysDicPubClassify;
            } else if (operateType === CFG.OPERATETYPE.SAVE) {
                operateFn = fetchSaveSysDicPubClassify;
            } else {
            ...
            if (data.errMessage) {
                ...
                yield put({
                    type: "mergeState",
                    payload: { sysDicPubClassifyFormVisible: false }
                });
                yield put({ type: "queryList" });
            }
        },
        ...
        *queryList({ payload }, { call, put, select }) {
            ...
            if (payload && payload.queryExtraBean) {//如果payload中有queryExtraBean则设置到state中
                ...
                queryExtraBean = yield select((state) => {
                    return state.sysDicPubClassifyModel.queryExtraBean;
                });
            ...
            if (payload && payload.queryConditionBean) { //如果payload中有queryConditionBean则设置到state中
                ...
                queryConditionBean = yield select((state) => {
                    return state.sysDicPubClassifyModel.queryConditionBean;
                });
            ...
            //fetch查询数据
            const data = yield call(fetchQuerySysDicPubClassify, queryExtraBean, realConditionBean);
